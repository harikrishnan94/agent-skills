...
| [`reduce-complexity`](skills/reduce-complexity/SKILL.md) | Finds accidental complexity in a PR or feature branch. Applies simplifications that pass its evidence and verification checks when asked to simplify. Questions and explicit report-only requests review without editing. Leaves edits uncommitted unless asked to commit. |
...
| [`humanize`](skills/humanize/SKILL.md) | Reduce unnecessary code and comment lines so reviewers can understand a change without the author's agent history. Removes planning leftovers and unexplained labels; preserves behavior, useful rationale, and established terminology. Fixes an existing diff by default. |
...
| [`plain-prose`](skills/plain-prose/SKILL.md) | Make prose a reader understands in one pass, non-native readers included: ten domain-neutral tests grounded in controlled-language standards and psycholinguistics — dependency distance, topic/stress position, an unfamiliar-term budget, no noun stacks, no private vocabulary. For commit messages, PR text and replies, Slack, mail, docs, and comments. Simplifies the sentence, never the content. |
| [`delegate-to-cursor`](skills/delegate-to-cursor/SKILL.md) | Offload the implementation typing of a settled plan to the Cursor CLI (`cursor-agent`) — reaching Grok, Gemini and GPT/Codex — while the calling agent keeps the planning and does the verification itself. Audits what the delegate actually did and flags claim/reality mismatches. |
| [`watch-pr`](skills/watch-pr/SKILL.md) | Watch a PR across CI rounds, attribute failures, validate and push authorized fixes, and hand off between agents. Includes a read-only observer. Requires 🕵 attribution on every GitHub comment it posts, including review replies; the marker is not used in PR/issue titles or bodies, or commit messages. Its pre-commit quality passes depend on the `humanize`, `plain-prose` and `reduce-complexity` skills. |
